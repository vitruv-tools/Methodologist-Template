...
public class LinkInsertedIntoSystemReaction extends AbstractReaction {
  private InsertEReference<EObject> insertChange;

  public LinkInsertedIntoSystemReaction(final Function<ReactionExecutionState, RoutinesFacade> routinesFacadeGenerator) {
  ...
  private static class Call extends AbstractRoutine.Update {
    ...
    public void updateModels(final InsertEReference insertChange, final tools.vitruv.methodologisttemplate.model.model.System affectedEObject, final EReference affectedFeature, final Link newValue, final int index, @Extension final Model2Model2RoutinesFacade _routinesFacade) {
      _routinesFacade.createAndInsertLink(affectedEObject, newValue);
    }
  }
  ...
  public boolean isCurrentChangeMatchingTrigger(final EChange change) {
    ...
    if (!_localTypedChange.getAffectedFeature().getName().equals("links")) {
    	return false;
    }
    if (!(_localTypedChange.getNewValue() instanceof tools.vitruv.methodologisttemplate.model.model.Link)) {
    	return false;
    }
  ...
  public void executeReaction(final EChange change, final ReactionExecutionState executionState, final RoutinesFacade routinesFacadeUntyped) {
    ...
    tools.vitruv.methodologisttemplate.model.model.Link newValue = (tools.vitruv.methodologisttemplate.model.model.Link)insertChange.getNewValue();
    int index = insertChange.getIndex();
    if (getLogger().isTraceEnabled()) {
    	getLogger().trace("Passed complete precondition check of Reaction " + this.getClass().getName());
    }
    
    new mir.reactions.model2Model2.LinkInsertedIntoSystemReaction.Call(executionState).updateModels(insertChange, affectedEObject, affectedFeature, newValue, index, routinesFacade);
  }
}
